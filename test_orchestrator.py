...
class MockService:
    ...
    async def handle_call(self, tool_name: str, parameters: dict) -> str:
        ...
        response = self.responses.get(
            tool_name, f"{self.name}: Unknown tool {tool_name}"
        )
...
async def test_service_integration():
    ...
    async def mock_call_service(
        service_name: str, tool_name: str, parameters: dict, session_context
    ):
    ...
    for command, expected_service in test_commands:
        ...
        result = await orchestrator.handle_enhanced_voice_command(
            text=command, user_id="test_user", interface_type="test"
        )
...
async def test_session_management():
    ...
    for i, session_id in enumerate(sessions):
        ...
        if context:
            print(
                f"   Session {i+1}: {len(context.command_history)} commands in history"
            )
        else:
