...
async def test_successful_operations():
    ...
    if available_devices:
        ...
        if active_device:
            logger.info(
                f"✓ Successfully activated device: {active_device.name} ({active_device.id})"
            )
        else:
        ...
        if zone_info and zone_info.is_active:
            logger.info(
                f"✓ Successfully activated zone: living_room (volume: {zone_info.volume}%)"
            )
        else:
    ...
    for zone_name, zone in audio_manager.zones.items():
        ...
        logger.info(
            f"  {zone_name}: devices={zone.devices}, volume={zone.volume}%, {status}"
        )
