...
class ElevenLabsProcessor(VoiceProcessorInterface):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if voices:
                ...
                logger.info(
                    f"ElevenLabs processor initialized with {len(voices)} voices"
                )
                return True
            else:
    ...
    async def text_to_speech(self, request: TTSRequest) -> TTSResponse:
        ...
        try:
            ...
            mock_audio_data = self._generate_mock_audio(
                request.text, request.voice_profile.sample_rate
            )
    ...
    async def speech_to_text(self, request: STTRequest) -> STTResponse:
        ...
        try:
            ...
            response = STTResponse(
                ...
                processing_time_ms=processing_time,
                segments=[
                    {"start": 0.0, "end": 2.5, "text": mock_text, "confidence": 0.95}
                ],
            )

            logger.info(f"--- OpenAI Whisper STT Completed Successfully ---")
            logger.info(
                f"Transcription: '{mock_text[:100]}{'...' if len(mock_text) > 100 else ''}'"
            )
...
class WakeWordDetector:
    ...
    def start_listening(self, callback: Callable[[str], None]) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Wake word detector started, listening for: {self.config.wake_words}"
            )
            return True
...
class VoiceActivityDetector:
    ...
    def __init__(self, config: VADConfig):
        ...
        logger.info(
            f"Voice Activity Detector created with aggressiveness: {config.aggressiveness}"
        )
    ...
    def process_frame(self, audio_frame: bytes) -> bool:
        ...
        try:
            ...
            if speech_active != self.is_active:
                ...
                logger.debug(
                    f"VAD state changed: {'SPEECH' if speech_active else 'SILENCE'}"
                )
...
class UnifiedVoiceProcessor:
    ...
    def setup_wake_word_detection(
        self, config: WakeWordConfig, callback: Callable[[str], None]
    ) -> bool:
    ...
    def get_engine_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "wake_word_active": self.wake_word_detector is not None
            and self.wake_word_detector.is_listening,
    ...
    def _get_voice_profile(
        self, voice_id: Optional[str], engine: VoiceEngine
    ) -> VoiceProfile:
...
if __name__ == "__main__":
    ...
    async def test_voice_processing():
        ...
        for voice in voices[:5]:  # Show first 5
            logger.info(
                f"  {voice.name} ({voice.id}) - {voice.engine.value} - {voice.gender.value}"
            )

        # Test TTS
        logger.info("\n--- Testing Text-to-Speech ---")
        tts_response = await processor.text_to_speech(
            "Hello, this is a test of the voice processing system.", speed=1.2
        )
        ...
        wake_config = WakeWordConfig(
            wake_words=["hey assistant", "computer"], sensitivity=0.7
        )

        def wake_word_callback(word: str):
            logger.info(f"Wake word detected: '{word}'")

        wake_success = processor.setup_wake_word_detection(
            wake_config, wake_word_callback
        )
