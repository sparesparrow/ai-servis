...
class MockUnifiedVoiceProcessor(UnifiedVoiceProcessor):
    ...
    async def initialize(
        self, elevenlabs_api_key: str = None, openai_api_key: str = None
    ) -> bool:
        ...
        try:
            ...
            if self.processors:
                self.initialized = True
                logger.info(
                    f"=== Mock Unified Voice Processor Initialized Successfully ==="
                )
...
async def test_individual_processors():
    ...
    if success:
        ...
        if tts_response.success:
            logger.info(
                f"✓ ElevenLabs TTS Success: {len(tts_response.audio_data)} bytes"
            )
    ...
    if success:
        ...
        tts_request = TTSRequest(
            text="Hello from OpenAI mock processor!", voice_profile=voice_profile
        )
        ...
        stt_request = STTRequest(
            audio_data=mock_audio, language="en", model="whisper-1"
        )
...
async def test_voice_activity_detection():
    ...
    config = VADConfig(aggressiveness=2, frame_duration_ms=30, sample_rate=16000)
    ...
    logger.info(
        f"✓ VAD processed 20 frames, detected speech in {speech_detections} frames"
    )
...
async def test_unified_voice_processor():
    ...
    for i, text in enumerate(test_texts):
        ...
        tts_response = await processor.text_to_speech(
            text=text, engine=engine, speed=1.0 + (i * 0.2), volume=0.8  # Vary speed
        )

        if tts_response.success:
            logger.info(
                f"✓ TTS Success ({engine.value}): {len(tts_response.audio_data)} bytes"
            )
            logger.info(f"  Duration: {tts_response.duration_ms}ms")
    ...
    for size in test_audio_sizes:
        ...
        stt_response = await processor.speech_to_text(
            audio_data=mock_audio, language="en", engine=VoiceEngine.OPENAI
        )
    ...
    wake_config = WakeWordConfig(
        wake_words=["hey computer", "assistant"], sensitivity=0.8
    )
...
async def test_error_handling():
    ...
    logger.info(
        f"Uninitialized TTS result: {tts_response.success}, error: {tts_response.error}"
    )

    stt_response = await processor.speech_to_text(b"\x00" * 1000)
    logger.info(
        f"Uninitialized STT result: {stt_response.success}, error: {stt_response.error}"
    )
    ...
    logger.info(
        f"Invalid engine TTS result: {mock_response.success}, error: {mock_response.error}"
    )
