...
class EnhancedNLPProcessor:
    ...
    async def parse_command(
        ...
        for intent, config in self.intent_patterns.items():
            score = self._calculate_intent_score(
                text_lower, words, intent, config, context
            )
            if score > 0:
                ...
                if context and "context_boost" in config:
                    boost_score = self._apply_context_boost(
                        config["context_boost"], context
                    )
                    if boost_score > 0:
        ...
        if best_intent in self.parameter_extractors:
            parameters = await self.parameter_extractors[best_intent](
                text_lower, words, context
            )
    ...
    def _apply_context_boost(
        self, boost_config: Dict[str, Any], context: SessionContext
    ) -> float:
    ...
    async def _extract_music_params(
        self, text: str, words: List[str], context: Optional[SessionContext]
    ) -> Dict[str, str]:
        ...
        if not params:
            query_words = [
                w for w in words if w not in ["play", "music", "song", "some"]
            ]
            if query_words:
    ...
    async def _extract_volume_params(
        self, text: str, words: List[str], context: Optional[SessionContext]
    ) -> Dict[str, str]:
    ...
    async def _extract_audio_params(
        self, text: str, words: List[str], context: Optional[SessionContext]
    ) -> Dict[str, str]:
    ...
    async def _extract_system_params(
        self, text: str, words: List[str], context: Optional[SessionContext]
    ) -> Dict[str, str]:
    ...
    async def _extract_hardware_params(
        self, text: str, words: List[str], context: Optional[SessionContext]
    ) -> Dict[str, str]:
    ...
    async def _extract_home_params(
        self, text: str, words: List[str], context: Optional[SessionContext]
    ) -> Dict[str, str]:
    ...
    async def _extract_file_params(
        self, text: str, words: List[str], context: Optional[SessionContext]
    ) -> Dict[str, str]:
    ...
    async def _extract_navigation_params(
        self, text: str, words: List[str], context: Optional[SessionContext]
    ) -> Dict[str, str]:
...
class ContextManager:
    ...
    def _load_contexts(self):
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(self.users_cache)} users and {len(self.sessions_cache)} sessions"
            )
...
class EnhancedCoreOrchestrator(MCPServer):
    ...
    async def handle_enhanced_voice_command(
        self,
        text: str,
        ...
        try:
            ...
            intent_result = await self.nlp_processor.parse_command(
            ...
            # Route command
            response = await self._route_enhanced_command(
                intent_result, session_context, context
            )
    ...
    async def handle_service_analytics(
        self, service_name: Optional[str] = None, metric: str = "response_time"
    ) -> Dict[str, Any]:
        """Handle service analytics request"""
        analytics = {}

        services_to_analyze = (
            [service_name] if service_name else list(self.services.keys())
        )
    ...
    async def _route_enhanced_command(
        self,
        ...
        result = await self._call_service_enhanced(
            service_name, intent, parameters, session_context
        )
        return result
    ...
    async def _handle_follow_up(
        ...
        return await self._route_enhanced_command(
            contextual_intent, session_context, None
        )
        ...
        try:
            ...
            if service.service_type == "mcp":
                result = await self._call_mcp_service(
                    service_name, tool_name, parameters
                )
