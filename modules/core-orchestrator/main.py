...
class NLPProcessor:
    ...
    def _extract_parameters(
        self, text: str, intent: str, words: List[str]
    ) -> Dict[str, Any]:
        ...
        if intent == "play_music":
            ...
            if not params:
                params["query"] = " ".join(
                    [w for w in words if w not in ["play", "music", "song"]]
                )
...
class CoreOrchestrator(MCPServer):
    ...
    async def _route_command(
        self, parsed_command: Dict[str, Any], context: Optional[Dict[str, Any]]
    ) -> str:
        ...
        if intent == "play_music":
            return await self._call_service(
                "ai-audio-assistant", "play_music", parameters
            )

        elif intent == "control_volume":
            return await self._call_service(
                "ai-audio-assistant", "set_volume", parameters
            )

        elif intent == "switch_audio":
            return await self._call_service(
                "ai-audio-assistant", "switch_audio_output", parameters
            )
            ...
            return await self._call_service(
                "ai-home-automation", "control_device", parameters
            )

        elif intent == "communication":
            return await self._call_service(
                "ai-communications", "send_message", parameters
            )

        elif intent == "navigation":
            return await self._call_service(
                "ai-maps-navigation", "get_directions", parameters
            )
    ...
    async def _call_service(
        self, service_name: str, tool_name: str, parameters: Dict[str, Any]
    ) -> str:
    ...
    async def register_service(
        self, service_name: str, host: str, port: int, capabilities: List[str]
    ):
    ...
    async def start_http_server(self, host: str = "0.0.0.0", port: int = 8080):
        ...
        async def handle_voice_command_http(
...
async def main():
    ...
    await orchestrator.register_service(
        "ai-audio-assistant", "localhost", 8082, ["audio", "music", "voice"]
    )
    await orchestrator.register_service(
        "ai-platform-linux", "localhost", 8083, ["system", "process", "file"]
    )
