...
class AudioManager:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.devices["speakers"] = AudioDevice(
                    "speakers", "Built-in Speakers", "speakers", True
                )
    ...
    def _discover_windows_devices(self):
        ...
        try:
            ...
            self.devices["speakers"] = AudioDevice(
                "speakers", "Default Speakers", "speakers", True
            )
            self.devices["headphones"] = AudioDevice(
                "headphones", "Default Headphones", "headphones", False
            )
        except Exception as e:
...
class AudioAssistantMCP(MCPServer):
    ...
    async def handle_play_music(
        self, query: str, source: str = "local", zone: Optional[str] = None
    ) -> str:
    ...
    async def handle_switch_audio_output(
        self, device: str, zone: Optional[str] = None
    ) -> str:
        ...
        try:
            ...
            return {
                ...
                "audio": {
                    ...
                    "available_devices": [
                        d.name for d in self.audio_manager.devices.values()
                    ],
                    "zones": {
                        name: {"volume": zone.volume, "active": zone.is_active}
